...
To try to demonstrate, this we're going to create a simple component which renders an input box where the user can enter a number. The component will then calculate 20% of the input value and write it back onto the screen like this:
...
The problem with this component is that it only does one thing, it is completely inflexible. If we wanted to also display 30% of a value then we would be unable to do so with this component. This is where composition comes in, by breaking your code into smaller parts it is possible to then compose those parts into solutions for different problems. Hooks allow us to do this very effectively, let's abstract the logic away from this component so that it can be reused in a second component to display the 30% value.
...
There are two principles which we should keep in mind whilst developing the solution for our problem, these are [DRY](https://en.wikipedia.org/wiki/Don%27t_repeat_yourself), _Don't Repeat Yourself_ which is aimed at reducing repetitive code and [YAGNI](https://en.wikipedia.org/wiki/You_aren%27t_gonna_need_it), _You Aren't Gonna Need It_, which is about not adding functionality until it is needed. Both of these principles work towards making code more maintainable. This isn't to say that you shouldn't be thinking about different ways in which your components may be used in the future; making a component that can easily be extended for use in other situations is very different to making a component which can already be used in many situations which don't (and may never) exist.

We are going to iterate over this component several times, each time adding further layers of abstraction. Ultimately you need to decide how far you go with the abstractions when writing your own components.
...
The hook is used by our original component like this:
...
We can now create the component which will give us the 30% value like so:

```js
import React from 'react';
import usePercentage from './usePercentage';

export default function GetThirtyPercent(props) {
...
```
The first instance of the component uses the `usePercentage` hook and displays the same message we've already seen. Below that it is used with a different hook, `useMultiplication`, for a different purpose: 

<ExampleComponent example="three" />

## Adding Defaults

You can see that now the component can be used for any number of different uses. There is, however, now a lot of configuration required each time the component is used. If the component is going to be used in a different way each time then this is fine, but if the component is going to be used to calculate 20% nine out of ten times and only used in a different way occasionally then it would be better if it were possible to do this: 
...
A big advantage of writing reusable code is that you can write one set of tests and feel happy that you have covered a good percentage of the functionality of your application. Splitting logic up also makes testing easier; if I am struggling to write a set of test I see that as an indication that the code that I am testing is probably doing too much and should possibly be broken down into smaller parts.
...
I have used [Jest](https://jestjs.io/) with [React Testing Library](https://testing-library.com/docs/react-testing-library/intro). I like React Testing Library although I'm not really using it in the way that it has been designed to be used here. The concept is that you should be using your component as your user might use it, however we are not doing that here. The main reason I use it is that it plays nicely with hooks whereas I have had problems getting other test libraries to work with them.

In order to test a hook, you need to use it in a component. What I do is create a component which uses the hook and passes the hook's values into a child component which is a mock function:
