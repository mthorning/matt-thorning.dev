...
title: 'First experience with Reason'
...
Reason is a new syntax based on OCaml which has a very good type system. For me, the syntax sits nicely between JavaScript and Rust which is ideal and the project was started by Jordan Walke which means the language also has first-class support for React! On top of this, the compiler is crazy fast and starting a new project takes mere seconds.

I thought it would be a good exercise to create myself a profile site with Reason, this is something I have been meaning to do anyway for a while but haven't gotten around to. I've spent the last week making the site, during which I have run into a couple of issues which I found difficult to resolve; the language is quite new so finding the information you need can be a little tricky at times, therefore I thought it would be a good idea to make a note of them (and my solutions to them) here.
...
If you spend some time looking at examples of code written in Reason you are likely to see three different pipe operators:
...
If the function you are piping to only takes a single argument then you can use either operator.

## Creating HTML lists

This seems like the right time to discuss creating HTML lists, `<ul>` and `<ol>`. It's quite common to have to build lists when you're creating a UI, if you use React a lot then you can probably do this with your eyes closed:
...
This is because `List.map` returns a list and not React elements which is expected to be returned from the `make` function. In order to get this to work we need to jump through some hoops, first we need to map through our list, then we need to convert our list to an array and finally we can convert that array to some React elements.
...
```reason
let make = _ => {
  let onChange = event => Js.log(event.target.value);
  <input onChange />;
}
```

Instead you need to pass the synthetic `event` to a function which will return an object of type Js.t, this is a type provided by Bucklescript which does not translate into a Reason record so Bucklescript provides its own syntax for accessing fields on a Js.t object:
...
The double hash (`##`) is the accessor which can be used to get any property which you would normally get from `event.target` when using JavaScript. It looks a bit strange when you first encounter it but I soon got used to it and find I don't have to think about what I need to type any more!
...
With both module types in the array, the lib folder get a js directory **and** and es6 directory. This means that the index.html file used for development can use the commonJs files from the js directory and the Webpack config for production can use the es6 files.

I also wanted to bundle the CSS files, I found the easiest way was to just add an index.js file to the CSS directory where I could import the css files, and then just add that to the Webpack `"entry"` array.
