...
Here's an example of some code where a reference is used after its value has gone out of scope:
...
The compiler is complaining because the variable `hello` is referencing a value (`greeting`) which is no longer in scope. Now look at this code:
...
Lifetime parameters are a kind of generic type. Rust's type inference is able to infer them in the body of functions but type inference is not allowed in function signatures so we need to annotate the lifetime parameters ourself, with a few exceptions.

Every reference has a lifetime parameter but we don't see them normally because, in certain situations, Rust is able to work them out for itself (this is known as Lifetime Elision and you can read about it [here](https://doc.rust-lang.org/book/ch10-03-lifetime-syntax.html?highlight=elision#lifetime-elision)). One of the rules for elision is that if there is only one input lifetime parameter then that lifetime is assigned to all output parameters. We need to break this rule for the purposes of this example in order to get the error which I want to talk about, hence the `unnecessary_argument`.

Here, we are using a function to get the `hello` value but the compiler is unable to work out that `greeting` doesn't live long enough because it doesn't have enough information about how the arguments to `get_output` and its return value are related to one another. This is an important point which warrants repeating:

<p style={{fontSize:'25px',backgroundColor:'#ffff0042', padding: '5px'}}>
  Lifetime parameters help the compiler fill in the blanks. They are a way of annotating a function signature to map the relationship between any references which are used by the function.
</p>
...
```
This function signature is now telling the compiler that the reference which is returned is borrowed from `main`. Notice that we have not added any lifetime parameters to `extra`, this is because it is irrelevant in this instance. It's quite common (and I am also guilty of this) to spam all references with a lifetime parameter when you get this error. Doing so in this case does make the error go away, and is technically correct because we are saying that `extra` lives _at least_ as long as `'a`, it's just that the compiler doesn't need to know about it, also it's good if we can only add the lifetime parameters which are needed because it makes us look like we know what we're doing!
...
Let's add some lifetimes to get our programme to compile:
...
```
The `struct` needs to declare a lifetime which is then used by each field, this says that the references on the fields need to at least live as long as the struct. The `impl` declares a lifetime which is then used by the struct. The lifetime also remains in scope for any functions in the block so we do not need to declare the lifetime parameter again on the `new` function, we can just use its signature. 
...
We may, however, want to make our signature more representative of what is actually happening and show that `name` has a different lifetime to `category`. 
...
found Thing<'_>
  ...
  Because we have told the compiler that `name` and `category` have different lifetimes, it can no longer be sure that both references will live for at least as long as the instance of `Thing`. We can let the compiler know that `'a` lasts for at least as long as `'b` like so: 
 ```ocaml
  impl<'a: 'b, 'b> Thing<'b> {
```
...
How did you get on? Let's work our way through this. We'll do the easy bit first, the IntStruct has one field which is a reference to an `i32`, we know that this reference is going to need to live for as long as the struct:
...
Remember the first change we made? We declared a lifetime parameter on `IntStruct`, now we just need to help the compiler by joining the dots:
